...
WORKDIR /src
COPY ["Mock.DocumentoscopyService.csproj", "Mock.DocumentoscopyService/"]
RUN dotnet restore "Mock.DocumentoscopyService/Mock.DocumentoscopyService.csproj"

WORKDIR "/src/Mock.DocumentoscopyService"
COPY . .
RUN dotnet build "Mock.DocumentoscopyService.csproj" --configuration Release --output /app/build

FROM build AS publish
RUN dotnet publish "Mock.DocumentoscopyService.csproj" \
...
Execute unit tests
```
    cd "Mock.DocumentoscopyService/test/Mock.DocumentoscopyService.Tests"
    dotnet test
```

Deploy application
```
    cd "Mock.DocumentoscopyService/src/Mock.DocumentoscopyService"
